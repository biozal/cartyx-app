...
vi.mock('~/components/sessions/SessionModal', () => ({
  SessionModal: ({ isOpen, session }: { isOpen: boolean; session?: unknown }) =>
    isOpen ? <div data-testid="session-modal">{session ? 'edit' : 'create'}</div> : null,
}));
...
vi.mock('@fortawesome/react-fontawesome', () => ({
  FontAwesomeIcon: ({ icon: _icon }: { icon: unknown }) => <span data-testid="fa-icon" />,
}));
...
function setupMocks(sessions = baseSessions, overrides?: { isOwner?: boolean }) {
...
describe('SessionsPage', () => {
  ...
  it('highlights the active session with border-l-[#2563EB] class', () => {
    ...
    const activeCard = screen
      .getByText('The Dark Descent')
      .closest('[data-testid="session-card-s1"]');
  ...
  it('shows back link to play page', () => {
    render(<SessionsPage />);
    const backLink =
      screen.getByRole('link', { name: /back/i }) || document.querySelector('a[href*="play"]');
    expect(backLink).toBeInTheDocument();
    expect(backLink).toHaveAttribute('href', expect.stringContaining('/campaigns/c1/play'));
  });
});
