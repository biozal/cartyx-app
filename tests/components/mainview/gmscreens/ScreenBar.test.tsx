...
describe('ScreenBar — settings dropdown', () => {
  ...
  it('clicking New Screen calls onCreateScreen and closes menu', async () => {
    ...
    await user.click(screen.getByTestId('screen-settings-trigger'));
    await user.click(
      within(screen.getByTestId('screen-settings-menu')).getByRole('menuitem', {
        name: /New Screen/i,
      })
    );
...
describe('ScreenBar — ARIA attributes', () => {
  ...
  it('each tab has a unique aria-controls matching its screen', () => {
    ...
    expect(screen.getByTestId('screen-tab-s1')).toHaveAttribute(
      'aria-controls',
      'gmscreen-tabpanel-s1'
    );
    expect(screen.getByTestId('screen-tab-s2')).toHaveAttribute(
      'aria-controls',
      'gmscreen-tabpanel-s2'
    );
    expect(screen.getByTestId('screen-tab-s3')).toHaveAttribute(
      'aria-controls',
      'gmscreen-tabpanel-s3'
    );
  });
