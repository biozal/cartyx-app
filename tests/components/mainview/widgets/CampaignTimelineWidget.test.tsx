...
const mockEvents: ReadonlyArray<Readonly<TimelineEvent>> = [
  ...
  },
  {
    ...
    summary:
      'Raiders from the reed marsh torched the market while the wizard tracked a stolen wardstone.',
  },
];
...
describe('CampaignTimelineWidget', () => {
  ...
  it('renders a horizontal scrollable container', () => {
    ...
    expect(
      scroll.querySelector('[data-layout="horizontal"][data-tone="current"]')
    ).toHaveTextContent('A reliquary opened beneath the chapel after the second toll.');
    ...
    const markerRows = scroll.querySelectorAll(
      '[data-layout="horizontal"] [data-part="timeline-marker-row"]'
    );
    ...
    expect(
      scroll.querySelectorAll('[data-layout="horizontal"] [data-part="timeline-marker"]')
    ).toHaveLength(mockEvents.length);
  });
