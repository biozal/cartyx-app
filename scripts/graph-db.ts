/**
 * The MongoDB driver's `db.collection(name)` interface over the graph, for E2E specs
 * and development fixtures.
 *
 * Specs and fixtures wrote collections directly through the driver. For a collection
 * that has moved to the graph, `collection(name)` answers the same calls — insertOne,
 * find().sort().toArray(), updateOne, bulkWrite… — in the driver's result shapes, through
 * the graph model the app itself uses, so a spec's writes obey the same schema and
 * unique keys. A collection that has not moved yet falls through to MongoDB while one is
 * given; that fallback goes with the last slice.
 *
 * Returned documents carry their `_id` as an ObjectId, as the driver's did, because
 * specs build filters and references from it.
 */
import { graphModels, type GraphModelName } from '../app/server/db/models/graph-models';
import { newObjectId } from '../app/server/repositories/collection';
...
type Model = (typeof graphModels)[GraphModelName];
